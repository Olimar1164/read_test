...
async def test_upload_and_chat(monkeypatch, tmp_path):
    ...
    client = SAIAConsoleClient('token','org','proj','assistant','https://api.saia.ai')
    ...
    with respx.mock(assert_all_called=False) as m:
        m.post("https://api.saia.ai/v1/files").mock(return_value=httpx.Response(200, json={"id":"file_123"}))
        m.post("https://api.saia.ai/chat").mock(return_value=httpx.Response(200, json={"choices":[{"message":{"content":"{\"message\": \"ok\"}"}}]}))
